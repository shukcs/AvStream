...
std::list<std::string> SplitString(const string &str, const string &sp, bool bSkipEmpty=false)
{
    std::list<std::string> strLsRet;
    int nSizeSp = sp.size();
    if (!nSizeSp)
        return strLsRet;
...
int64_t str2int(const std::string &str, unsigned radix=10, bool *suc=NULL)
{
...
static uint32_t str2ipv4(const string &ip)
{
    ...
    int i = 0;
    for (const string &itr : SplitString(ip, "."))
    {
        u.b[i++] = (unsigned char)str2int(itr);
    }
    return u.l;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
